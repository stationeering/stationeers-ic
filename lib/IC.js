...
module.exports = class IC {
  ...
  _instruction_alias(fields) {
    ...
    if (matches) {
      ...
      if (foundAlias) {
        this._aliases[fields[0]] = {
          value: foundAlias.value,
          type: foundAlias.type
        };
  ...
  _validateLine(content, line) {
    ...
    if (jumpTagMatch) {
      if (this._jumpTags[jumpTagMatch[1]] !== line) {
        errors.push({
          line: line,
          error: "INVALID_JUMP_TAG_DUPLICATE",
          "type": "error"
        });
      }
      ...
      if (tokens.length > 1) {
        errors.push({
          line: line,
          error: "INVALID_JUMP_TAG_CONTENT_AFTER",
          "type": "error"
        });
      }
  ...
  getInstructions() {
    return Object.keys(this._opcodes).reduce((acc, opcode) => {
      acc[opcode] = {
        category: this._opcodes[opcode].category,
        fields: this._opcodes[opcode].fields
      };
      return acc;
    }, {});
  }
